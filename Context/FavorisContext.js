...
export const FavorisProvider = ({ children }) => {
		...
		const loadFavoris = async () => {
			...
			try {
				if (user) {
					...
					const savedFavoris = await AsyncStorage.getItem("favoris_guest");
					if (savedFavoris) {
	...
	useEffect(() => {
		const syncFavoris = async () => {
			...
			try {
				...
				await AsyncStorage.setItem(`favoris_${user.email}`, JSON.stringify(favoris));
			}
		};
	...
	useEffect(() => {
		const saveLocalFavoris = async () => {
			if (user) return; // Ne pas sauvegarder localement si connecté

			try {
				await AsyncStorage.setItem("favoris_guest", JSON.stringify(favoris));
			} catch (error) {
	...
	const toggleFavoris = useCallback((article, user) => {
		if (!user) {
